<script lang="ts">
	...
	import Block from './chart/Block.svelte';
	import Element from './chart/Element.svelte';
	import MultiVerticalArrow from './chart/MultiVerticalArrow.svelte';
	import VerticalArrow from './chart/VerticalArrow.svelte';
</script>
...
<div class="mx-auto max-w-xl">
	<div class="flex flex-col justify-center mx-4 max-2xl:text-sm">
		...
		<div class="h-6"><VerticalArrow showArrow /></div>
		<Element title="Linear" onHover={()=>focus.set({
			...$focus,
			name: 'linear',
		  })} />
		<div class="h-6"><VerticalArrow showArrow /></div>
		<Block onHover={()=>focus.set({
			linesHighlighted:[],
			name: 'block',
		  })}>
		...
		<div class="h-6"><VerticalArrow showArrow/></div>
		<Element title="Positional Encoding" color="#d8b4fe" onHover={()=>focus.set({
			...$focus,
			name: 'encoding',
		  })} />
		<div class="h-6"><VerticalArrow showArrow /></div>
		<Element title="Embedding" color="#fca5a5"onHover={()=>focus.set({
			...$focus,
			name: 'embedding',
		  })}/>
		<div class="h-6"><VerticalArrow showArrow /></div>
