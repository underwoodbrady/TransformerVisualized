<script lang='ts'>
    ...
    focus.subscribe((f)=>{
        ...
        if((focusOptions.includes(f.name)))
            currentFocus = f.name; 
        else
            currentFocus = 'transformer'
    })
...
<div
	class="w-full border-2 border-neutral-800 hover:border-white duration-150 rounded-md bg-neutral-600 p-4 mx-auto max-w-xl cursor-text"
>
